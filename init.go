...
func parseTOMLConfig(configFile string, conf *configs.Config) error {
	...
	conf.CaptchaStdio = getTOMLVal(confTOML.CaptchaStdio, false)
	conf.ForceIPv4 = getTOMLVal(confTOML.ForceIPv4, false)
...
func init() {
	...
	flag.BoolVar(&conf.CaptchaStdio, "captcha-stdio", false, "yibinu-patch: exchange image captcha over stdin/stdout using the @CAPTCHA:/@CAPTCHA_ANSWER: line protocol (for GUI wrapper usage)")
	flag.BoolVar(&conf.ForceIPv4, "force-ipv4", false, "yibinu-patch: restrict all outgoing dials (HTTP + tunnel) to IPv4")
